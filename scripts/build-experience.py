...
nyc_openapi = {
 ...
 "servers": [{"url": "https://api.nyc.gov", "description": "Illustrative unified gateway host (design-first)."},
             {"url": f"{SITE}/experience/api", "description": "Static reference API — example data at stable URLs, reads only (GitHub Pages)."}],
 "x-apis-io": {
   "mcp": {"definition": f"{RAW}/experience/nyc-mcp.json", "note": "NYC-wide MCP gateway (design-first)."},
   "mcpInstall": "https://install.apicommons.org/?server=" + RAW + "/experience/mcp-server/server.json",
   "mcpPackage": "@api-common/nyc-mcp",
   "referenceApi": {"base": f"{SITE}/experience/api", "catalog": f"{SITE}/experience/api/index.json",
                    "note": "Example data synthesized from the agency schemas; GET-callable, reads only."},
