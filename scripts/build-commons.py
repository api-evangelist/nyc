...
md.append("")
md.append(f"**Reference implementation:** {', '.join('`'+m+'`' for m in out['totals']['migratedDomains'])} are migrated to `$ref` the canonical set (back-compatible — the `$defs` names are unchanged). The rest still carry private copies; migrating them is tracked in the [roadmap](ROADMAP.md).\n")
